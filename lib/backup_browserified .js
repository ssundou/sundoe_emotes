...
addChatMessage = function(platform, nick, message, data) {
  ...
  nick = '<img src="./images/resouce/'+ platform +'.png"></img>' + nick;
  ...
  if (data) {
    ...
    if (data.badges && configData.badgeVisible) {
      if ('chzzk' == platform) {
        data.badges.forEach(function(badge) {
          ...
          chatBadge.src = badge.imageUrl;  // URL 직접 할당
          chatBadge.classList.add("badge_custom"); // 커스텀 클래스
          chatBadgeBox.appendChild(chatBadge);
        });
      }
      else if ('twitch' == platform) {
        // 뱃지 채워넣고
    ...
    if (data.cheers != undefined) {
      if ('chzzk' == platform) {
        let chzzkCheeseMsg = "☆ {!0:{bits} 치즈 }후원 ! ☆"
        chatLowerBox.innerHTML =
          '<div class="chat_cheer_box">' +
          replaceMsgFormat(chzzkCheeseMsg, data.cheers) +
          "</div>" + chatLowerBox.innerHTML;
      }
      else if('twitch' == platform) {
        // 비트 메세지도 그 위에 추가하고
...
var concatChatMessage = function(platform, nick, message, data) {
...
var applyMessage = function(message, data) {
  ...
  message = chzzkReplaceEmojis(message, data.chzzk_emojis)
...
var checkComplete = function() {
  ...
  if (++completeCount == num) {
    ...
    var chat = addChatMessage('twitch', "",
  ...
  if (configData.debugLevel != 0) {
    if (configData.debugLevel == 1) { debugLog = function(dat) { console.log(dat); }; }
      ...
      debugLog = function(dat, unConcat) {
        if (unConcat) {
          addChatMessage("twitch", "DEBUG", dat,
            { badges:["moderator/1"], color:"red", escape:false });
        }
        else {
          concatChatMessage("twitch", "DEBUG", dat,
            { badges:["moderator/1"], color:"red", escape:false });
        }
      };
    }
    ...
    dcConScript.onload = function() {
      if (dcConsData.length == 0) { debugLog("디씨콘을 불러오는 데 실패했습니다."); }
        ...
        applyDcCon = function(message, data) {
          let imgNum = 0;
          message = '<div class="' + chatNumber + ' modify_type"><first>' + message;
          for (var index in keywords) {
            var keyword = keywords[index];

            if (message.indexOf('커져라!') != -1 ) {
              message = message.replace('커져라!', '');
              message = message.replace('modify_type', 'modify_type bbig animate__animated animate__pulse');
            }

            if (message.indexOf('두근두근!') != -1 ) {
              message = message.replace('두근두근!', '');
              message = message.replace('modify_type', 'modify_type animate__animated animate__infinite	infinite animate__heartBeat faaastest');
            }

            if (message.indexOf('빙글빙글!') != -1 ) {
              message = message.replace('빙글빙글!', '');
              message = message.replace('modify_type', 'modify_type binglebingle');
            }

            if (message.indexOf('등장!') != -1 ) {
              message = message.replace('등장!', '');
              message = message.replace('modify_type', 'modify_type animate__animated animate__zoomInDown');
            }

            if (message.indexOf('도망쳐!') != -1 ) {
              message = message.replace('도망쳐!', '');
              message = message.replace('modify_type', 'modify_type animate__animated animate__bounceOutRight animate__slow');
            }

            if (message.indexOf('사실그거마미조임') != -1 ) {
              message = message.replace('사실그거마미조임', '');
              message = message.replace('modify_type', 'change_mamijo');
            }

            if (message.indexOf('~으랏~챠!') == -1
              && message.indexOf('으랏~챠!') != -1 
              && (message.indexOf(':') != -1 || message.indexOf("~" + keyword) != -1)) {
              let dcConInfo = dcConsData.find(function(element) {
                return element.keywords.indexOf('으랏~챠!') != -1;
              });

              message = message.replace('으랏~챠!', '');
              message = message.replace('modify_type', 'modify_type jjibu animate__animated animate__bounce');
              let src = configData.dcConsURI + dcConsSubURI + dcConInfo.name + '?n=' + chatNumber;
              message = message.replace('<first>', '<img class="dccon notani" src="' + src + '?v" />');
              let audio = new Audio(configData.dcConsURI + dcConsSubURI + dcConInfo.soundURL);
              audio.autoplay = 1;
              audio.volume = 0.3;
              audio.load();
            }

            message.replace('modify_type', '');
            ...
            if (message.indexOf("~" + keyword) != -1) {
              ...
              if(1 == dcConInfo.isSound) {
                let audio = new Audio(configData.dcConsURI + dcConsSubURI + dcConInfo.name);
                audio.autoplay = 1;
                audio.volume = 0.3;
                audio.load();
              }
              else {
                if (Array.isArray(dcConInfo.name)) {
                  let src = configData.dcConsURI + dcConsSubURI + dcConInfo.name[Math.ceil(Math.random() * dcConInfo.name.length) - 1] + '?n=' + chatNumber;
                  message = message.split("~" + keyword).join('<img class="dccon" src="' + src +'" />');
                }
                else {
                  let src = configData.dcConsURI + dcConsSubURI + dcConInfo.name + '?n=' + chatNumber;
                  message = message.split("~" + keyword).join('<img class="dccon" src="' + src + '" />');
                ...
                if (false == isEmpty(dcConInfo.soundURL)){
                  if (Array.isArray(dcConInfo.soundURL)) {
                    let audio = new Audio(configData.dcConsURI + dcConsSubURI + dcConInfo.soundURL[Math.ceil(Math.random() * dcConInfo.soundURL.length) - 1]);
                    audio.autoplay = 1;
                    audio.volume = 0.3;
                    audio.load();
                  }
    ...
    lines.forEach( function(element) {
      ...
      // 트위치 명령어 처리
        ...
        case "PRIVMSG":     // 채팅 수신
          ...
          // 메세지 출력
          addChatMessage('twitch', realNick, message, data);
          break;
...
function chzzkReplaceEmojis(message, emojis) {
  ...
  for (const emojiKey in emojis) {
    const emojiHTML =
      ...
      emojiRegex = new RegExp('{:' + emojiKey + ':}', 'g');
    updatedMessage = updatedMessage.replace(emojiRegex, emojiHTML);
  }
